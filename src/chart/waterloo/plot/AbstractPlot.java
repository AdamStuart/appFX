...
public abstract class AbstractPlot<T extends List<? extends Node>> extends StackPane implements ListChangeListener<Number>
{
	...
	private final StringProperty xData = new StyleableStringProperty("")
	{
		...
		@Override public CssMetaData<? extends Styleable, String> getCssMetaData()
		{
	...
	BooleanProperty dataPolar = new StyleableBooleanProperty(false)
	{
		...
		@Override public void set(boolean tf)
		{
	...
	@SuppressWarnings("unchecked") public List<Node> getAllPlots()
	{
	...
	public final Point2D getData(Chart chart, double x, double y)
	{
		...
		{
			// The Plotting methods always use cartesian coordinates, so convert
			// to those
	...
	public final Point2D getInverse(Chart chart, double x, double y)
	{
		...
		{
			// Plotting is always done using cartesian coordinates, so convert
			// to those
			x = Math.cos(x) * y;
			y = Math.sin(x) * y;
			return chart.getAxisSet().getInverse(x, y);
		}
		return Point2D.ZERO;// zeroPoint;//Point2D.ZERO;//Not recognized by
							// javac in Java 8eas?
	}
	 ...
	 */
	@Override public final void layoutChildren()
	{
		arrangeLabels();
	...
	private static class StyleableProperties
	{
		...
		private static final CssMetaData<AbstractPlot, MARKERTYPE> MARKERSTYLE = new CssMetaData<AbstractPlot, MARKERTYPE>("-w-plot-marker-style",
						(StyleConverter<?, MARKERTYPE>) StyleConverter.getEnumConverter(MARKERTYPE.class), MARKERTYPE.CIRCLE)
		{

			@Override public boolean isSettable(AbstractPlot node)
			{
				return node instanceof MarkerInterface;
			}

			@Override public StyleableProperty<MARKERTYPE> getStyleableProperty(AbstractPlot node)
			{
				return (StyleableProperty<MARKERTYPE>) node.visualModel.markerType;
			}

		};
		private static final CssMetaData<AbstractPlot, Number> MARKERRADIUS = new CssMetaData<AbstractPlot, Number>("-w-plot-marker-radius", StyleConverter.getSizeConverter(), 5d)
		{
			...
			@Override public StyleableProperty<Number> getStyleableProperty(AbstractPlot node)
			{
				return (StyleableProperty<Number>) node.visualModel.markerRadius;
			}
		};
		private static final CssMetaData<AbstractPlot, String> XDATA = new CssMetaData<AbstractPlot, String>("-w-plot-xdata", StyleConverter.getStringConverter(), "")
		{

			@Override public boolean isSettable(AbstractPlot n)
			{
				return true;
			}

			@Override public StyleableProperty<String> getStyleableProperty(AbstractPlot n)
			{
				return (StyleableProperty<String>) n.xData;
			}
		};
		private static final CssMetaData<AbstractPlot, Paint> FILL = new CssMetaData<AbstractPlot, Paint>("-w-plot-marker-fill", StyleConverter.getPaintConverter(), Color.GRAY)
		{

			@Override public boolean isSettable(AbstractPlot node)
			{
				return node instanceof MarkerInterface && node.visualModel.fill != null && !node.visualModel.fill.isBound();
			}

			@SuppressWarnings("unchecked") @Override public StyleableProperty<Paint> getStyleableProperty(AbstractPlot node)
			{
				return (StyleableProperty<Paint>) node.visualModel.fill;
			}
		};
		private static final CssMetaData<AbstractPlot, Paint> EDGECOLOR = new CssMetaData<AbstractPlot, Paint>("-w-plot-marker-edgecolor", StyleConverter.getPaintConverter(), Color.DARKGREY)
		{

			@Override public boolean isSettable(AbstractPlot node)
			{
				return node instanceof MarkerInterface && node.visualModel.getEdgeColor() != null && !node.visualModel.edgeColorProperty().isBound();
			}
		...
		private static final CssMetaData<AbstractPlot, Number> EDGEWIDTH = new CssMetaData<AbstractPlot, Number>("-w-plot-marker-edgewidth", StyleConverter.getSizeConverter(), 1d)
		{

			@Override public boolean isSettable(AbstractPlot node)
			{
				return node instanceof MarkerInterface && node.visualModel.edgeWidth != null && !node.visualModel.edgeWidth.isBound();
			}
		...
		private static final CssMetaData<AbstractPlot, String> YDATA = new CssMetaData<AbstractPlot, String>("-w-plot-ydata", StyleConverter.getStringConverter(), "")
		{
		...
		private static final CssMetaData<AbstractPlot, Paint> LINECOLOR = new CssMetaData<AbstractPlot, Paint>("-w-plot-line-color", StyleConverter.getPaintConverter(), Color.DARKGREY)
		{

			@Override public boolean isSettable(AbstractPlot node)
			{
				return node instanceof LineInterface && node.getLineColor() != null && !node.visualModel.lineColor.isBound();
			}
		...
		private static final CssMetaData<AbstractPlot, Number> BASEVALUE = new CssMetaData<AbstractPlot, Number>("-w-plot-basevalue", StyleConverter.getSizeConverter(), 0d)
		{
			...
			@Override public StyleableProperty<Number> getStyleableProperty(AbstractPlot n)
			{
				return (StyleableProperty<Number>) n.baseValue;
			}
		};
		private static final CssMetaData<AbstractPlot, Number> LINEWIDTH = new CssMetaData<AbstractPlot, Number>("-w-plot-line-width", StyleConverter.getSizeConverter(), 1d)
		{

			@Override public boolean isSettable(AbstractPlot node)
			{
				return node instanceof MarkerInterface && node.visualModel.lineWidth != null && !node.visualModel.lineWidth.isBound();
			}
		...
		private static final CssMetaData<AbstractPlot, Boolean> DATAPOLAR = new CssMetaData<AbstractPlot, Boolean>("-w-plot-datapolar", StyleConverter.getBooleanConverter(), false)
		{
			...
			@Override public StyleableProperty<Boolean> getStyleableProperty(AbstractPlot n)
			{
				return (StyleableProperty<Boolean>) n.dataPolar;
			}
		};

		private static final CssMetaData<AbstractPlot, String> EASTDATA = new CssMetaData<AbstractPlot, String>("-w-plot-eastdata", StyleConverter.getStringConverter(), "")
		{
		...
		private static final CssMetaData<AbstractPlot, String> NORTHDATA = new CssMetaData<AbstractPlot, String>("-w-plot-northdata", StyleConverter.getStringConverter(), "")
		{
		...
		private static final CssMetaData<AbstractPlot, String> WESTDATA = new CssMetaData<AbstractPlot, String>("-w-plot-westdata", StyleConverter.getStringConverter(), "")
		{
		...
		private static final CssMetaData<AbstractPlot, String> SOUTHDATA = new CssMetaData<AbstractPlot, String>("-w-plot-southdata", StyleConverter.getStringConverter(), "")
		{
		...
		private static final CssMetaData<AbstractPlot, String> LABELDATA = new CssMetaData<AbstractPlot, String>("-w-plot-labels", StyleConverter.getStringConverter(), "")
		{
	...
	public class VisualModel
	{
		...
		private final StyleableObjectProperty<Paint> lineColor = new StyleableObjectProperty<Paint>(Color.GRAY)
		{
			...
			@Override public CssMetaData<? extends Styleable, Paint> getCssMetaData()
			{
		...
		private final ObjectProperty<MARKERTYPE> markerType = new StyleableObjectProperty<MARKERTYPE>(MARKERTYPE.CIRCLE)
		{
			...
			@Override public void set(MARKERTYPE type)
			{
				super.set(type);
				switch (type)
				{
			...
			@Override public CssMetaData<? extends Styleable, MARKERTYPE> getCssMetaData()
			{
		 ...
		 *
		 * @param index
		 *            of the marker to retrieve
		 * @return the marker
		 */
		public Node getMarker(int index)
		{
			if (index > markerArray.size() - 1) for (int k = markerArray.size() - 1; k < index; k++)
		 ...
		 * @param marker
		 *            the new Marker which may be any JavaFX Node
		 */
		...
		/**
		 * @param dynamicMarkerSize
		 *            the dynamicMarkerSize to set
		 */
