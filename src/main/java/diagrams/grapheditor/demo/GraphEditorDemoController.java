 ...
 */
package diagrams.grapheditor.demo;
...
import diagrams.grapheditor.demo.customskins.DefaultSkinController;
import diagrams.grapheditor.demo.customskins.GreySkinController;
import diagrams.grapheditor.demo.customskins.SkinController;
import diagrams.grapheditor.demo.customskins.TreeSkinController;
import diagrams.grapheditor.demo.customskins.grey.GreySkinConstants;
import diagrams.grapheditor.demo.customskins.tree.TreeConnectorValidator;
...
import diagrams.grapheditor.window.WindowPosition;
...
/**
 * Controller for the {@link AppGraphEditorDemo} application.
 */
public class GraphEditorDemoController
{
	...
	@FXML private MenuItem clearConnectorsButton;
	@FXML private Menu connectorTypeMenu;
	@FXML private Menu connectorPositionMenu;

	@FXML private RadioMenuItem inputConnectorTypeButton;
	@FXML private RadioMenuItem outputConnectorTypeButton;
	@FXML private RadioMenuItem leftConnectorPositionButton;
	@FXML private RadioMenuItem rightConnectorPositionButton;
	@FXML private RadioMenuItem topConnectorPositionButton;
	@FXML private RadioMenuItem bottomConnectorPositionButton;
	@FXML private RadioMenuItem showGridButton;
	@FXML private RadioMenuItem snapToGridButton;
	@FXML private RadioMenuItem defaultSkinButton;
	@FXML private RadioMenuItem treeSkinButton;
	@FXML private RadioMenuItem greySkinButton;
	@FXML private Menu intersectionStyle;
	@FXML private RadioMenuItem gappedStyleButton;
	@FXML private RadioMenuItem detouredStyleButton;
	@FXML private Menu zoomOptions;
	...
	@FXML public void save()		{		graphEditorPersistence.saveToFile(graphEditor);	}
	@FXML public void clearAll()	{		Commands.clear(graphEditor.getModel());	}
	...
	@FXML public void cut()	{		graphEditor.getSelectionManager().cut();	}
	@FXML public void copy()	{		graphEditor.getSelectionManager().copy();	}
	@FXML public void paste()	{		activeSkinController.get().handlePaste();	}
	@FXML public void selectAll()	{		graphEditor.getSelectionManager().selectAll();	}
	@FXML public void deleteSelection()	{		graphEditor.getSelectionManager().deleteSelection();	}
	@FXML public void addNode()	{		activeSkinController.get().addNode(currentZoomFactor);	}

	@FXML public void addConnector()	{
		activeSkinController.get().addConnector(getSelectedConnectorPosition(),
						inputConnectorTypeButton.isSelected());
	}

	@FXML public void clearConnectors()	{		activeSkinController.get().clearConnectors();	}
	@FXML public void setDefaultSkin()	{		activeSkinController.set(defaultSkinController);	}
	@FXML public void setTreeSkin()	{		activeSkinController.set(treeSkinController);	}
	@FXML public void setGreySkin()	{		activeSkinController.set(greySkinController);	}

	@FXML public void setGappedStyle()
	{
		graphEditor.getProperties().getCustomProperties().remove(SimpleConnectionSkin.SHOW_DETOURS_KEY);
		graphEditor.reload();
	}

	@FXML public void setDetouredStyle()
	{
	...
	@FXML public void toggleMinimap()
	{
	 ...
	 * </p>
	 */
	public void panToCenter()	{		graphEditorContainer.panTo(WindowPosition.CENTER);	}
	...
	private void initializeMenuBar()
	{
		...
		positionGroup.getToggles()
						.addAll(leftConnectorPositionButton, rightConnectorPositionButton);
		positionGroup.getToggles()
						.addAll(topConnectorPositionButton, bottomConnectorPositionButton);
	...
	private void initializeZoomOptions()
	{
		...
		double[] zooms = new double[] { 0.1, 0.25, 0.5, 1, 2, 4 };
		for (int i = 1; i <= 5; i++)
		{
			final RadioMenuItem zoomOption = new RadioMenuItem();
			final double zoomFactor = zooms[i];

			zoomOption.setText((int) (100 * zooms[i]) + "%");
			...
			if (i == 3)	zoomOption.setSelected(true);
		}
	}
	...
	private void setZoomFactor(final double zoomFactor)
	{
		...
		graphEditorContainer.panTo(currentCenterX * zoomFactorRatio, currentCenterY
						* zoomFactorRatio);
	...
	/**
	 * Crudely inspects the model's first node and sets the new skin type
	 * accordingly.
	 */
	...
	/**
	 * Checks if the connector buttons need disabling (e.g. because no nodes are
	 * selected).
	 */
	private void checkConnectorButtonsToDisable()
	{

		final boolean nothingSelected = graphEditor.getSelectionManager().getSelectedNodes()
						.isEmpty();
	...
	private void flushCommandStack()
	{
		...
		final EditingDomain editingDomain = AdapterFactoryEditingDomain
						.getEditingDomainFor(graphEditor.getModel());
	...
	/**
	 * Gets the side corresponding to the currently selected connector position
	 * in the menu.
	 *
	 * @return the {@link Side} corresponding to the currently selected
	 *         connector position
	 */
	private Side getSelectedConnectorPosition()
	{
		if (leftConnectorPositionButton.isSelected())			return Side.LEFT;
		if (rightConnectorPositionButton.isSelected())			return Side.RIGHT;
		if (topConnectorPositionButton.isSelected())			return Side.TOP;
		return Side.BOTTOM;
	}
}
