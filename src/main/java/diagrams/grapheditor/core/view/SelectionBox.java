package diagrams.grapheditor.core.view;
...
public class SelectionBox extends Rectangle {
    ...
    private void addWindowFocusListener() {
        ...
        sceneListener = (observable, oldValue, newValue) -> {
            if (oldValue != null) {   oldValue.windowProperty().removeListener(windowListener);      }
            if (newValue != null) {    newValue.windowProperty().addListener(windowListener);        }
        };

        windowListener = (observable, oldValue, newValue) -> {
            if (oldValue != null) {    oldValue.focusedProperty().removeListener(windowFocusListener);    }
            if (newValue != null) {    newValue.focusedProperty().addListener(windowFocusListener);       }
        };

        windowFocusListener = (observable, oldValue, newValue) -> {    if (!newValue) {  setVisible(false);  }   };
