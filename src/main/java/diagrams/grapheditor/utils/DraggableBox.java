...
public class DraggableBox extends StackPane {
    ...
    protected void storeClickValuesForDrag(final double x, final double y) {
        ...
        lastParentWidth =  (container != null && container.getWidth() > 0) ? container.getWidth() : Double.MAX_VALUE; 
        lastParentHeight =  (container != null && container.getHeight() > 0) ? container.getHeight() : Double.MAX_VALUE; 
        absoluteMaxWidth =  (container != null && container.getMaxWidth() > 0) ? container.getMaxWidth() : Double.MAX_VALUE; 
        absoluteMaxHeight =  (container != null && container.getMaxHeight() > 0) ? container.getMaxHeight() : Double.MAX_VALUE; 
    ...
    private Region getContainer(final Node node) {
        ...
        if (parent instanceof Region)   return (Region) parent;
