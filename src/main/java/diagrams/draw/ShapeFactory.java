...
public class ShapeFactory {
	...
	public Shape makeLabeledShape(Tool tool, AttributeMap attrMap, String s) {
    	...
    	text.layoutXProperty().bind(ctr.centerXProperty().subtract(30));	// width / 2
    	text.layoutYProperty().bind(ctr.centerYProperty().subtract(10));
		return newShape;
	}
...
//		text.setBoundsType(TextBoundsType.VISUAL);
	...
	protected class CircleMouseHandler extends NodeMouseHandler {
		...
		protected void handleMouseMoved(final MouseEvent event) {
			super.handleMouseMoved(event);
			Circle c = (Circle) target;
			if (onEdge(currentPoint, c))	c.setCursor(Cursor.H_RESIZE);
	...
	protected class PolygonMouseHandler extends NodeMouseHandler {
		...
		protected void handleMouseMoved(final MouseEvent event) {
			super.handleMouseMoved(event);
			Polygon p = (Polygon) target;
			if (onVertex(currentPoint, p) >= 0)	p.setCursor(Cursor.H_RESIZE);
...
//			dragLine.setStartY(event.getY());
	...
	public class NodeMouseHandler implements EventHandler<MouseEvent> {
		...
		ContextMenu buildContextMenu() {
			...
			MenuItem dup = makeItem("Duplicate", a -> 		{	getController().duplicateSelection();	});
			MenuItem del = makeItem("Delete", a -> 			{	getController().deleteSelection();	});
			MenuItem toFront = makeItem("Bring To Front", a -> 	{	getController().toFront();	});
			MenuItem toBack = makeItem("Send To Back", a -> 	{	getController().toBack();	});
			MenuItem group = makeItem("Group", a -> 		{	getController().group();	});
			MenuItem ungroup = makeItem("Ungroup", a -> 	{	getController().ungroup();	});
					
//			MenuItem dup = 		new MenuItem("Duplicate");	
//			dup.setOnAction();
//			MenuItem del = new MenuItem("Delete");
//			del.setOnAction(a -> 	{	getController().deleteSelection();		});
//			MenuItem toFront = new MenuItem("Bring To Front");
//			toFront.setOnAction(a -> {	getController().toFront();	});
//			MenuItem toBack = new MenuItem("Send To Back");
//			toBack.setOnAction(a -> {	getController().toBack();	});
//			MenuItem group = new MenuItem("Group");
//			group.setOnAction(a -> 	{	getController().group();	});
//			MenuItem ungroup = new MenuItem("Ungroup");
//			ungroup.setOnAction(a -> {	getController().ungroup();	});
			menu.getItems().addAll(toFront, toBack, group, ungroup, dup, del);
			return menu;
		}
		
		MenuItem makeItem(String name, EventHandler foo)
		{
