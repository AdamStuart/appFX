...
public class ColorChooserSample extends Application 
{
  ...
  public void start(final Stage stage) throws Exception {
    ...
    stage.setTitle("Color Chooser");		  // initialize the stage.
    stage.initStyle(StageStyle.UTILITY);
    final ColorChooser colorChooser = new ColorChooser(); 
    final Scene scene = new Scene(colorChooser, 80, 320);
...
class ColorChooser extends VBox 
{
  ...
  public Color getChosenColor() {    return chosenColor.get();  }
  public ReadOnlyObjectProperty<Color> chosenColorProperty() {    return chosenColor.getReadOnlyProperty();  }
  ...
  public String getChosenColorName() {    return chosenColorName.get();  }
  
  /** Preferred size for a web palette tile */
  private DoubleProperty prefTileSize = new SimpleDoubleProperty(MIN_TILE_SIZE);
 
  //---------------------------------------------------------------------------------------
  public ColorChooser() {    this(webPalette);  }
  
  public ColorChooser(ColorPool pool) 
  {   
	  this(pool.asStringPairs());  
	  pool.setChooser(this);;
  }
  GridPane swatch;
...
//---------------------------------------------------------------------------------------
  public void setColors(Object[] newColors)
  {
	 ...
	 int idx = 0;
	 ObservableList<Node> nodes = swatch.getChildren();
	  for (Node node : nodes)
	  {
		  if (idx < nColors && node instanceof Button)
		  {
			  Color c = (Color) newColors[idx++];
			  String webstr = colorToString( c);
...
//---------------------------------------------------------------------------------------
	...
	public ColorChooser(String[][] colors) {
		...
		// calculate the number of columns and rows based on the number of
		// colors and a golden ratio for layout.
		nColumns = 8; // Math.floor(Math.sqrt(colors.length) * 2 /  GOLDEN_RATIO);
		nRows = Math.ceil(colors.length / nColumns);

		createTheButtons(colors);
		((Button) swatch.getChildren().get(0)).fire();		//		// select the first color in the chooser.
	...
	//---------------------------------------------------------------------------------------
	private void createTheButtons(String[][] colors) {
		...
		for (String[] namedColor : colors) {
			...
			final Button colorChoice = new Button();				// create a button for choosing a color.
			colorChoice.setUserData(colorName);
 
      GridPane.setRowIndex(colorChoice,    i / (int) nColumns);		    // position the button in the grid.
