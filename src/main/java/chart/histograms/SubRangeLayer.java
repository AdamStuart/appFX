...
//		selectionRectangle.setFill(null);
	...
	private void addDragSelectionMechanism() {
		pane.setOnMousePressed((event) -> {
			...
			if (selectionH.isVisible() && selectionH.testHit(event.getX()))
			{
				startPt = event.getX();
			} else
			{
		...
		pane.setOnMouseDragged((event) -> {
			...
			if (startPt >= 0)
			{
				double delta = event.getX() - startPt;
				moveSelection(delta);
				selectionH.setLayoutX(selectionH.getLayoutX() + delta);
				startPt = event.getX();
			}
			else
			{
