...
public class QuadTree {
    ...
    private static class Node {
        ...
        private Node(final Word w) {      word = w;        }
    }
    ...
    private void addByVertix(final Word word, int x2, int y2, Node node) {
        ...
        if(x2 < x) {
            if(y2 < y) {
                if(node.NW == null)     node.NW = new Node(word);
                 else         			add(word, node.NW);
            } else {
                if(node.SW == null)  	node.SW = new Node(word);
                 else                   add(word, node.SW);
            }
        } else {
            if(y2 < y) {
            	if(node.NE == null)     node.NE = new Node(word);
                 else                   add(word, node.NE);
            } else {
                if(node.SE == null)     node.SE = new Node(word);
                 else                   add(word, node.SE);
    ...
    private void getNearbyByVertix(Word word, int x2, int y2, Node node, Set<Word> nearby) {
        ...
        if(x2 < x) {
            if(y2 < y) {
                if(node.NW != null)        getNearby(word, node.NW, nearby);
            } else {
                if(node.SW != null)        getNearby(word, node.SW, nearby);
            }
        } else {
            if(y2 < y) {
                if(node.NE != null)        getNearby(word, node.NE, nearby);
            } else {
                if(node.SE != null)        getNearby(word, node.SE, nearby);
            }
        }
    }
