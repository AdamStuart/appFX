    ...
    private void installMouseListeners() {
        ...
        view.setOnMouseMoved((MouseEvent m2) -> {
            ...
            if (isInnerAxisPainted()) {
                ...
                if (atOrigin(x, y)) {      view.setCursor(Cursor.HAND);       return;                } 
                if (onXAxis(y)) {          view.setCursor(Cursor.E_RESIZE);      return;                } 
                if (onYAxis(x)) {          view.setCursor(Cursor.N_RESIZE);      return;                } 
             }

            view.setCursor(isMousePositionDisplayed() ? Cursor.CROSSHAIR : Cursor.DEFAULT);
