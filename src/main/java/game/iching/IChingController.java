...
public class IChingController
{
	...
	// pick a random number 0-63, break it into 8 x 8 matrix
	// Construct the two trigrams from the binary, and use them
	// to look up the appropriate passage from pages[] array.
		...
		AnimationTimer  timer = new AnimationTimer() {
          ...
          @Override public void handle(long now) {
        	  if (now - lastUpdate >= TIMER50MILLI) 
              {
	    			...
	    			VBox hex = makeHexagramBox(idx, i == tries);
	...
	private VBox makeHexagramBox(int idx, boolean verbose)
	{
		...
		return mom; 					// makeTriagramTest();
	}

	private VBox makeTriagramTest()		// spit out the 8 triagrams in order
	{
		VBox box = new VBox(32);
		for (int i = 0; i < 8; i++)
			box.getChildren().add(makeTrigram(i));
		return box;
	}
...
//		System.out.println(idx  + ": makeTrigram");
	...
	{
		int top = map(idx / 8);			// see the chart at top of the web page
		int btm = map(idx % 8);
